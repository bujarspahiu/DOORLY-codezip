...
import { adminGetUsers, adminCreateUser, adminUpdateStatus, adminDeleteUser, adminGetActivity, adminGetStats } from '../../lib/database';
...
const AdminPanel: React.FC = () => {
  ...
  const [userForm, setUserForm] = useState({ name: '', email: '', company: '', plan: 'starter' });
  ...
  const loadActivity = async () => {
    try { const logs = await adminGetActivity(); setActivityLog(logs); }
  ...
  const toggleUserStatus = async (id: string, currentStatus: string) => {
    ...
    try {
      await adminUpdateStatus(id, newStatus as any);
      setUsers(users.map(u => u.id === id ? { ...u, status: newStatus } : u));
    } catch (err) { console.error(err); }
  };

  const deleteUser = async (id: string) => {
    try { await adminDeleteUser(id); setUsers(users.filter(u => u.id !== id)); }
    catch (err) { console.error(err); }
  };
  ...
  const addUser = async () => {
    setSaving(true);
    try {
      await adminCreateUser(userForm.email, userForm.name, userForm.company, userForm.plan);
      setShowUserForm(false);
      setUserForm({ name: '', email: '', company: '', plan: 'starter' });
      await loadData();
    } catch (err) { console.error(err); }
  ...
  const totalRevenue = businessUsers.reduce((s, u) => s + (u.revenue || 0), 0);
  ...
  const navItems = [
    ...
    { id: 'activity', label: lang === 'en' ? 'Activity Log' : 'Aktiviteti', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg> },
  ];
    ...
    <div className="min-h-screen bg-gray-900 flex">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-gray-800 border-r border-gray-700 transform transition-transform duration-200 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="flex flex-col h-full">
          ...
          <div className="px-4 py-4 border-t border-gray-700"><div className="flex gap-2"><button onClick={() => setLang(lang === 'en' ? 'al' : 'en')} className="flex-1 py-2 text-xs font-medium text-gray-300 bg-gray-700 rounded-lg hover:bg-gray-600">{lang === 'en' ? 'Shqip' : 'English'}</button><button onClick={logout} className="flex-1 py-2 text-xs font-medium text-red-400 bg-gray-700 rounded-lg hover:bg-gray-600">{lang === 'en' ? 'Logout' : 'Dilni'}</button></div></div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 p-4 sm:p-6 overflow-y-auto">
          {activeTab === 'dashboard' && (
            <div className="space-y-6">
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  { label: lang === 'en' ? 'Total Users' : 'Gjithsej Përdorues', value: businessUsers.length.toString(), sub: `${activeUsers} active`, color: 'from-blue-500 to-blue-600' },
                  { label: lang === 'en' ? 'Total Quotes' : 'Gjithsej Oferta', value: (platformStats.totalQuotes || 0).toString(), sub: 'All time', color: 'from-green-500 to-green-600' },
                  { label: lang === 'en' ? 'Total Invoices' : 'Gjithsej Fatura', value: (platformStats.totalInvoices || 0).toString(), sub: 'All time', color: 'from-purple-500 to-purple-600' },
                  { label: lang === 'en' ? 'Platform Revenue' : 'Të Ardhurat', value: `€${(totalRevenue / 1000).toFixed(0)}K`, sub: 'Combined', color: 'from-orange-500 to-red-500' },
                ].map((stat, i) => (
              ...
              <div className="bg-gray-800 rounded-xl border border-gray-700 p-6">
                <h3 className="text-lg font-bold text-white mb-4">{lang === 'en' ? 'Top Users by Revenue' : 'Përdoruesit Kryesorë'}</h3>
                {businessUsers.length === 0 ? <p className="text-gray-500 text-sm">{lang === 'en' ? 'No users yet' : 'Ende pa përdorues'}</p> : (
                  <div className="space-y-3">
                    {[...businessUsers].sort((a, b) => (b.revenue || 0) - (a.revenue || 0)).slice(0, 5).map((u, i) => (
                      <div key={u.id} className="flex items-center justify-between">
                        <div className="flex items-center gap-3"><span className="w-6 text-center text-sm font-bold text-gray-500">#{i + 1}</span><div><p className="text-sm font-medium text-white">{u.company_name || u.full_name}</p><p className="text-xs text-gray-400">{u.email}</p></div></div>
                        <span className="text-sm font-bold text-green-400">€{(u.revenue || 0).toLocaleString()}</span>
                      </div>
                    ))}
                  </div>
                )}
          ...
          {activeTab === 'users' && (
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <button onClick={() => setShowUserForm(true)} className="flex items-center gap-2 px-5 py-2.5 bg-red-600 text-white rounded-xl font-semibold hover:bg-red-700">
              ...
              {showUserForm && (
                <div className="bg-gray-800 rounded-xl border border-gray-700 p-6">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    ...
                    <input type="email" value={userForm.email} onChange={(e) => setUserForm({ ...userForm, email: e.target.value })} placeholder="Email" className="px-4 py-2.5 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:ring-2 focus:ring-red-500" />
                  ...
                  <div className="flex justify-end gap-3 mt-4">
                    ...
                    <button onClick={addUser} disabled={saving} className="px-5 py-2 bg-red-600 text-white rounded-lg font-semibold hover:bg-red-700 disabled:opacity-50">{saving ? '...' : (lang === 'en' ? 'Create' : 'Krijo')}</button>
                  </div>
                </div>
              )}
              <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead><tr className="bg-gray-700/50"><th className="px-6 py-3 text-left text-xs font-semibold text-gray-400 uppercase">{lang === 'en' ? 'User' : 'Përdoruesi'}</th><th className="px-6 py-3 text-left text-xs font-semibold text-gray-400 uppercase">{lang === 'en' ? 'Company' : 'Kompania'}</th><th className="px-6 py-3 text-left text-xs font-semibold text-gray-400 uppercase">{lang === 'en' ? 'Plan' : 'Plani'}</th><th className="px-6 py-3 text-left text-xs font-semibold text-gray-400 uppercase">Status</th><th className="px-6 py-3 text-left text-xs font-semibold text-gray-400 uppercase">{lang === 'en' ? 'Revenue' : 'Të Ardhura'}</th><th className="px-6 py-3 text-right text-xs font-semibold text-gray-400 uppercase">{lang === 'en' ? 'Actions' : 'Veprime'}</th></tr></thead>
                    <tbody className="divide-y divide-gray-700">
                      {loadingUsers ? (
                        <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-500">{lang === 'en' ? 'Loading...' : 'Duke ngarkuar...'}</td></tr>
                      ) : filteredUsers.length === 0 ? (
                        <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-500">{lang === 'en' ? 'No users found' : 'Asnjë përdorues'}</td></tr>
                      ) : filteredUsers.map((u) => (
                        <tr key={u.id} className="hover:bg-gray-700/50 transition-colors">
                          <td className="px-6 py-4"><p className="text-sm font-medium text-white">{u.full_name}</p><p className="text-xs text-gray-400">{u.email}</p></td>
                          <td className="px-6 py-4 text-sm text-gray-300">{u.company_name || '-'}</td>
                          ...
                          <td className="px-6 py-4 text-sm text-green-400 font-mono font-semibold">€{(u.revenue || 0).toLocaleString()}</td>
          ...
          {activeTab === 'plans' && (
            <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {[
                  { name: 'Starter', price: 29, count: businessUsers.filter(u => u.plan === 'starter').length, features: ['50 quotes/month', '1 user', 'Basic calculator', 'PDF export'] },
                  { name: 'Professional', price: 79, count: businessUsers.filter(u => u.plan === 'professional').length, features: ['Unlimited quotes', '5 users', 'Custom branding', 'Invoice management'] },
                  { name: 'Enterprise', price: 199, count: businessUsers.filter(u => u.plan === 'enterprise').length, features: ['Everything in Pro', 'Unlimited users', 'API access', 'White-label'] },
                ].map((plan, i) => (
          ...
          {activeTab === 'activity' && (
            <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-700"><h3 className="font-bold text-white">{lang === 'en' ? 'Recent Activity' : 'Aktiviteti i Fundit'}</h3></div>
              {loadingActivity ? (
                <div className="p-8 text-center text-gray-500">{lang === 'en' ? 'Loading...' : 'Duke ngarkuar...'}</div>
              ) : activityLog.length === 0 ? (
                <div className="p-8 text-center text-gray-500">{lang === 'en' ? 'No activity yet' : 'Ende pa aktivitet'}</div>
              ) : (
                <div className="divide-y divide-gray-700">
                  {activityLog.map((log, i) => (
                    <div key={i} className="px-6 py-4 flex items-center gap-4 hover:bg-gray-700/50 transition-colors">
                      <div className="w-2 h-2 rounded-full bg-green-400 flex-shrink-0" />
                      <div className="flex-1">
                        <p className="text-sm text-white"><span className="font-semibold">{log.user_name || 'User'}</span> — {log.action}</p>
